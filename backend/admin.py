...
)  # Must be service role for admin
...
def send_sms(phone, code, base_url):
    ...
    try:
        message = twilio_client.messages.create(body=body, from_=TWILIO_FROM, to=phone)
...
if __name__ == "__main__":
    ...
    invite_parser.add_argument(
        "--send", action="store_true", help="Send SMS via Twilio"
    )
    invite_parser.add_argument(
        "--url", default="http://localhost:3000", help="Base URL for the link"
    )
